...
def get_specific_instructions(evaluation_method: str, patient_data: dict, lab_results: list[dict]) -> str:
    ...
    if "NEHRAZENY" in evaluation_method.upper():
        ...
        has_abnormality = any(res.get("interpretation_status") not in ["NORMAL", "N/A"] for res in lab_results if res.get("parameter_code") not in ["HCG_CODE", "PSA_CODE", "KO_GROUP_CODE", "MS_GROUP_CODE"]) # Nahradit skutečnými kódy
        # if not has_abnormality and not any(code in evaluation_method for code in ["HCG", "PSA", "KO", "MS"]): # Zjednodušená kontrola
            # return "NEHRAZENÝ POPIS: Všechny sledované parametry (mimo explicitních výjimek) jsou v referenčním rozmezí. Popis se negeneruje."

    # Výjimky, které se vždy popisují
    # TODO: Identifikovat HCG, PSA, KO, M+S na základě parameter_code nebo evaluation_method
    if "HCG" in evaluation_method.upper() or any(res.get("parameter_name", "").upper() == "HCG" for res in lab_results):
    ...
    if "PSA" in evaluation_method.upper() or any(res.get("parameter_name", "").upper() == "PSA" for res in lab_results):
        instructions.append("PSA: Vždy generuj popis. Vyjádři se k výsledku vzhledem k věku pacienta a jeho stavu. Zohledni kumulativní hodnocení.")
    if "KO" in evaluation_method.upper() or any("KREVNÍ OBRAZ" in res.get("parameter_name", "").upper() for res in lab_results): # Zjednodušená detekce
        instructions.append("KREVNÍ OBRAZ (KO): Vždy doplň text, i když je v normě (např. 'Krevní obraz v normě.').")
    if "M+S" in evaluation_method.upper() or any("MOČ + SEDIMENT" in res.get("parameter_name", "").upper() for res in lab_results): # Zjednodušená detekce
        instructions.append("MOČ + SEDIMENT (M+S): Vždy doplň text, i když je v normě (např. 'Moč a sediment v normě, bez známek močové infekce.').")

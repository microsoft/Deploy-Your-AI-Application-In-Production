...
if (-not $virtualMachineId) {
    ...
    Write-Host "3. Run the following command to process & ingest the sample data:"
    Write-Host "powershell -ExecutionPolicy Bypass -File process_sample_data.ps1 -SearchEndpoint '$env:AZURE_SEARCH_ENDPOINT' -OpenAiEndpoint '$env:AZURE_OPENAI_ENDPOINT' -EmbeddingModelName '$env:EMBEDDING_MODEL_NAME' -EmbeddingModelApiVersion '2025-01-01-preview' -UseLocalFiles $true"
} else {
    ...
    Write-Host "4. Run the following commands one by one to process & ingest the sample data:"

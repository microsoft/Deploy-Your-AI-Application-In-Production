...
if [ -z "$virtualMachineId" ]; then
  ...
  echo "2. Navigate to the scripts directory: cd \"$(dirname "$0")\""
  echo "3. Run the following command to process the sample data:"
  echo "powershell -ExecutionPolicy Bypass -File process_sample_data.sh -SearchEndpoint '$AZURE_SEARCH_ENDPOINT' -OpenAiEndpoint '$AZURE_OPENAI_ENDPOINT' -EmbeddingModelName '$EMBEDDING_MODEL_NAME' -EmbeddingModelApiVersion '2025-01-01-preview' -UseLocalFiles \$true"
else
